...
**First real capture (2026-09-02), via a genuine USB-tethered Web Inspector session (not a photo) — hypothesis (a) mostly refuted, hypothesis (b) gained real support, but the actual "miniature" symptom wasn't reproduced this run:**

- The ~7.2× scale-mismatch readings during the cold-start `LIMITED/INITIALIZING` window (17s this run — unusually slow, worth watching separately) were confirmed to be exactly what §4/§8 already understood: garbage pre-convergence data, correctly rejected by the gate. The USER PHYSICALLY MEASURED the printed target at 3×3cm, matching `physicalTargetWidthMeters: 0.03` exactly — ruling out a print-scale mistake as the explanation for these readings. The instant `trackingStatus` reached `NORMAL`, the very next sample read `ratio=1.18` and was correctly accepted, revealing the scene. **The gate is working exactly as designed; this is not where the bug lives.**
- A 52-second stretch (+57.8s to +110.1s) logged NOTHING at all — no image events, no trackingStatus change, no marker flicker — while presumably the phone kept moving. Every tracking-decision signal this file's telemetry (§5–§27) has ever added is tied to an IMAGE event or a status change; ordinary SLAM/VIO world-tracking POSITION drift, which can accumulate even while `trackingStatus` stays `NORMAL` (the status reflects internal tracking confidence, not absolute-position accuracy), would produce exactly this silent gap. This is now the leading candidate for hypothesis (b).
- **This specific capture did NOT reproduce "anchor lost / scale miniature"** — confirmed directly by the person testing: movement was minimal (limited by the literal USB cable required for Web Inspector), and the symptom "happened in a couple of past iterations," not this one. So this capture rules one thing out and strengthens a hypothesis, but is not itself the smoking-gun capture.

**The tooling gap this exposed: Web Inspector requires a USB tether, which caps exactly the kind of movement (a real walkaround) needed to reproduce the bug.** Fixed by adding a full-session log EXPORT to the existing `?debug=1` inline console (`public/index.html`), not a new debugging channel: every line already mirrored to the on-screen panel is also pushed into an uncapped (20000-line) in-memory buffer, and a small "Show log" button (the only element in this overlay with `pointer-events:auto` — everything else stays `none` on purpose, per this file's own earlier note about not blocking AR-scene taps) opens a real, native `<textarea>` with the full session pre-selected — solving two limits of the on-screen panel at once: its 200-line rolling window, and its `pointer-events:none` making its own text uncopiable by touch. Best-effort `navigator.clipboard.writeText()` runs too, but the textarea is the guaranteed path, not a fallback of last resort. No cable, no Mac, no Web Inspector needed — walk the full loop, then tap the button.

**Next step, unchanged in kind, now unblocked in practice:** one real untethered walkaround with `?debug=1`, reproduce "loses anchor / scale miniature," tap "Show log," send the full text. Specifically worth watching for: any silent multi-second gap (a fixed candidate for hypothesis (b)) that lines up with when the symptom was actually seen, and whether that gap correlates with distance from every one of the 4 plaques (further supporting "no re-detection available to correct drift" as the actual mechanism, not a code defect in the gate itself).

---

## 28. The real blocker to getting a usable capture: an unthrottled per-frame console warning, found from a second real capture (2026-09-02)

A second attempt at a full-session copy-paste (Web Inspector, still cable-limited — same session, another cold start captured) hit a hard character limit before the log left the first ~13 seconds of `trackingStatus=LIMITED/INITIALIZING`. The cause wasn't the anchor at all: `logSampleRejected()`'s trackingStatus-rejection `console.warn` had no throttle of its own, unlike `warnIfScaleMismatch()` right above it in the same function — so every single `'updated'` frame during a sustained non-NORMAL stretch printed an almost-identical line, roughly 60 times per second. A cold start stuck in `INITIALIZING` for 13-30+ seconds (as both real captures so far have shown — see §27, this was NOT a one-off slow session) produces 800-1800+ copies of one line before anything diagnostically interesting even has a chance to appear, in either the on-screen `?debug=1` panel (200-line cap, blown through in under 4 seconds at that rate) or a copy-pasted/exported log (any fixed character budget).

**This was never a mystery about the anchor — it was a console-flooding bug in the logging itself, and it has likely been silently sabotaging every capture attempt in this investigation so far (§27's captures included), not just this one.**

**Fix (`ImageTargetAnchorSource.ts`):** the trackingStatus-rejection warning now shares the exact same 1/s throttle (`SCALE_MISMATCH_WARN_INTERVAL_MS`, its own timestamp variable) as the scale-mismatch warning beside it. Worst case during a fully rejected stretch is now ~2 warning lines/second (one per throttle, when both conditions are true) instead of up to ~120/second. Also corrected a stale comment on that same throttle constant, which had claimed the rejection path "only runs during the brief pre-freeze convergence window... a few samples at most" — directly contradicted by both real captures now on record (13s and 17s cold starts respectively, hundreds of rejected samples each).

**Verified in software:** `npm run typecheck`/`build`/`test` clean, 53/53 (no test asserted on warning frequency; nothing to update). **Requires the physical exhibit, not verifiable in software:** whether a full walkaround capture is now actually completable end-to-end (on-screen panel and "Show log" export alike) — the real test of this fix is simply whether the NEXT capture reaches the part of the session that matters instead of drowning in cold-start noise again.

---

## 29. Third real capture (2026-09-02, same day, throttle fix confirmed working) — the symptom was seen with ZERO image-target telemetry in the preceding 24 seconds; ruled out ordinary distance/perspective directly with the person testing; added the one signal still missing

The §28 throttle fix worked as intended — this capture ran cleanly to 53+ seconds (previous two died inside the first 13-17s) with continuous `HotspotProjector`/`MarkerLayer` activity throughout, confirming the render loop and camera were live the whole time, not frozen or crashed.

**The critical fact:** from the last image event at `+29.0s` (a trustworthy re-detection, immediately followed by continuous marker activity as the camera panned) to `+53.2s` — when the person testing noticed the model had already shrunk and pressed "Show log" — there is **not one single `FOUND`/`updated`/`LOST` line, and no `TrackingStatus` change.** 24 seconds, confirmed continuously live (via the marker transitions) at no point during which any image target was in view, or any tracking-status transition occurred. The anchor's transform was therefore untouched for that entire window — whatever "shrunk" was not caused by any accepted or rejected pose sample, because none were evaluated at all.

**Directly asked and confirmed, not assumed:** whether this could simply be normal perspective (walking far enough from a real, meter-scale, fixed-in-place anchor makes it look smaller, correctly). The person testing stayed relatively close — not far enough that shrinking would be the expected, correct behavior of a working anchor. This rules out "not a bug, just distance" and leaves genuine mid-session divergence between the camera's reported pose and physical reality as the remaining explanation — hypothesis (b) from §27, now with a real capture's silence as supporting (not yet conclusive) evidence.

**The gap in the instrumentation this exposed:** every camera-position log added so far (§27) only fires from inside `ImageTargetAnchorSource`, on an APPLIED pose — which by construction can never fire during a stretch with no image events at all. The exact window that mattered here logged nothing about the camera, because nothing about an image target happened in it.
...
## 31. §30's validation test, run (2026-09-02): re-detection DOES correct the drift — confirmed with a live capture; a second, separate, real bug found in the same walk (back/left plaques never detected once)

**§30's question, answered — the correction mechanism works.** `docs/log-2.txt` (1158 lines) captures the test directly: at `+90.0s`, `site-tracking-right` fires `FOUND` for the first time all session —

```
[+90.0s] [ImageTarget] FOUND "site-tracking-right"
  scale=0.091m pos=(-1.09, -0.00, -1.23) rot=(0.04, 0.71, 0.68, -0.18)
  trackingStatus=NORMAL
  first sighting of a NEW plaque — applied unconditionally, firing onOriginChanged
[+90.0s] [ImageTargetAnchorSource]  camera=(-1.16, 0.66, -1.17) anchor=(0.07, 0.23, 0.13) dist=1.84m
```

— and the model visibly snapped to the newly-computed position (confirmed both by the person testing and by the log: 6 hotspot markers immediately project to sane, in-frame screen coordinates in the following lines, not garbage). Reported experience: "el modelo salta, pero hacia el nuevo target, como si cambiara su punto de anclaje" — **this is the correction working exactly as designed, not a new bug.** The anchor WAS in the wrong (drifted) place; a fresh sighting recomputed its true position using that plaque's own offset/rotation composition (§E) and snapped there — a discontinuity is the expected, correct look of fixing a real positional error without smoothing across it (this project deliberately snaps discontinuities exactly rather than lagging toward them — see `resetPoseFilters`'s removed-filter history and the §26 "discrete re-detection" reasoning). §30's fork is resolved: the correction path itself is NOT broken; a proactive coaching nudge (encourage glancing at a plaque before drift becomes obvious) is now the well-evidenced next UX step, not a fresh code investigation.

**A second, independent, real bug found in the same walk: `site-tracking-back` and `site-tracking-left` produced ZERO `FOUND` events in the entire 1158-line/~140-second capture**, while `site-tracking-front` fired 31 times and `site-tracking-right` once. Ruled out as a code/build defect before suspecting the environment: `ImageTargetAnchorSource.acquire()`'s own log confirms both names were correctly registered and awaited (`"waiting for first imagefound of any of \"site-tracking-front\", \"site-tracking-back\", \"site-tracking-left\", \"site-tracking-right\"..."`), and a direct file comparison of all 4 compiled targets (`public/assets/image-targets/site-tracking-{front,back,left,right}/`) shows identical structure — same 720×720 resolution, `isRotated: false`, same build pipeline, same day. **No difference at the code/asset level explains the gap.**

**Confirmed directly with the person testing, not assumed:** two contributing physical factors, not mutually exclusive — (1) the back/left printed targets looked visibly worse (print quality/contrast) and that side of the table had worse lighting than front/right, and (2) the walking path that session didn't pass as close to / as square-on to back/left as it did to front/right. Both are asset-authoring/physical-setup and content-design issues (per this project's own tracking-quality guidance, `docs/asset-authoring-guide.md` §3.1 and 8th Wall's own docs, already cited in §22 — poor contrast and oblique/distant viewing angle both directly degrade feature-point detection, regardless of engine) — **not a software bug**, and not fixed by more changes to `ImageTargetAnchorSource`.

**Next steps, two independent tracks:**
1. **Physical, no code:** re-inspect the back/left printed sheets side-by-side with front/right for print quality/contrast (re-print if visibly worse — same 100mm-calibration-ruler check as `tools/plaque/site-tracking/print-sheet.html` already prescribes), and check/improve lighting on that side of the physical setup.
2. **UX, code, well-evidenced now (not yet built):** a proactive coaching nudge — if no re-detection has occurred for some threshold of time (or of estimated drift), hint the user to glance at any nearby plaque, using the same debounced-hint pattern `TrackingLossHint` already established. This directly mitigates BOTH findings from this entry: it recovers from ordinary SLAM drift before it's visually obvious (§30), and it increases the chances of ever detecting a harder-to-see plaque (back/left) by encouraging a deliberate, closer, square-on look rather than relying on an incidental glance while walking past.

---

## 32. A rotation question raised, then superseded by a stronger finding: SLAM's OWN scale calibration drifts too, and it can make our own protective gate block recovery for a full minute

**Context:** after §31, the person testing fixed back/left print/lighting and reported a NEW symptom on the next walk (`docs/log-3.txt`): the model appeared with a diagonal tilt from the very first scan of `front`, never corrected. Investigation of that capture found exactly ONE trustworthy pose accepted in the entire 93-second session — every later `front` re-detection was scale-rejected, and back/left/right were never detected at all despite the print fix. Since only one sample ever governed the scene's orientation, two hypotheses were equally consistent with one data point: (a) `TARGET_FRAME_TO_WORLD_FIX`'s fixed rotation convention (still flagged in its own doc comment as unconfirmed on real 8th Wall hardware) or the physical mount angle is systematically off, or (b) `isSampleTrustworthy()` checks scale and trackingStatus but never rotation plausibility, so a single noisy rotation reading slipped through. A discriminating test was proposed: deliberately re-scan `front` several times in a row and see if the tilt reproduces identically (→ (a)) or varies (→ (b)).

**That test (`docs/log-4.txt`, run next) surfaced something more fundamental before the rotation question could even be answered.** Reported: scanning any of the 4 plaques now feels very difficult, and after the first successful scan of `front`, it was never possible to re-scan it. The capture shows this precisely: `front` fires `FOUND` roughly 60+ times over the next 62 seconds — image recognition itself works fine, continuously — and **every single one of those 60+ detections was rejected**, not primarily by `trackingStatus`, but by the scale-mismatch check. The rejected ratios tell the story on their own: starting around 9s post-bootstrap they're wildly inconsistent and high (2.06, 2.26, 3.26, 4.14, 5.72, 6.00, 20.11 — ordinary unconverged cold-start noise, already understood). But from roughly `+25s` onward, the pattern changes character entirely: the ratios go **below 1 and decline in a smooth, consistent trend** — 0.56, 0.58, 0.54, 0.52 clustering together, then continuing down to 0.30, 0.27, 0.41, 0.45, 0.48 — the engine's own measurement of the SAME physical, unmoving 30mm target shrinking steadily over roughly 40 seconds, not bouncing around randomly. Only at `+71.1s` did a sample happen to land back inside tolerance (ratio 1.09) and finally get accepted.

**Interpretation, not a guess:** this is a THIRD facet of the same root phenomenon §27/§30 already measured in position (camera height climbing ~1.5m/45s) — 8th Wall's `scale:'absolute'` calibration is not just failing to converge at cold start, it continues drifting throughout a live session, and evidently this can affect the engine's own internal notion of what "one meter" means, not merely the camera's position estimate. The practical, user-facing consequence is worse than the position-drift case: **once the scale calibration has drifted far enough, `isSampleTrustworthy()` — correctly protecting the anchor from a genuinely bad reading — also blocks every subsequent, otherwise-valid re-detection of the SAME target, for as long as the drift persists.** The gate cannot currently distinguish "this one sample is noise" from "the whole coordinate system's scale has shifted and every reading will look 'wrong' by a similar amount until it un-drifts or someone recenters." This reframes "difícil de escanear" and "nunca logré volver a escanearla": it was never a print/lighting/detection problem this time — the plaque was seen dozens of times — it was the trust gate doing exactly its documented job against a moving target it has no way to recognize as such.

**Not yet acted on — two structurally different next moves, deliberately not chosen between yet:**
1. **Environmental, no code:** the person testing was asked directly whether the room around the table is visually plain/repetitive (known to worsen monocular VIO drift) or richly textured — answered "not sure / a mix," inconclusive. A controlled comparison (same setup, a room with unambiguously more visual texture/objects) would test this empirically where self-assessment couldn't.
...
## 34. §33's fix confirmed working on real hardware; the rotation question exposed a real gap in the logging itself, not more drift

**§33 confirmed.** The very next capture (`docs/log-6.txt`) shows the fix doing exactly its job: `site-tracking-right` (ratio 2.71), `site-tracking-back` (ratio 2.11), and `site-tracking-left` (attempted 6 times, ratios 15.19 to 49.90) all logged `"first sighting of a NEW plaque, but scale implausible REJECTED"` — every one of these would have unconditionally corrupted the anchor before the fix. The anchor held on the session's one good sample (ratio 0.96) throughout. Reported directly: the model's SCALE looked correct this time — consistent with the fix working — while an ORIENTATION problem (a perceived diagonal tilt) and the underlying scale-instability symptom both persisted.

**The persisting "scale loss" is the SLAM engine itself, already understood, not a new gap.** This single session's rejected ratios swing from 105.34 down to below 1 and back up to 49.90 within a few hundred seconds — the same volatile absolute-scale calibration §27/§30/§32/§33 already characterized. The gate is now correctly protecting the anchor from all of it (§33); it cannot make the underlying calibration itself stop drifting — that remains an engine/environment property, not a bug this codebase can fix by tuning a check further.

**The rotation question (open since §32) hit a real instrumentation gap, not a dead end.** The one sample this session that passed the gate (ratio 0.96) should have been the perfect data point to compare against earlier sessions' tilts — except its actual rotation was never printed anywhere. The `[ImageTarget] updated (throttled 1/s)` line only shows the RAW incoming sample once per second; the accept/reject decision runs on every `'updated'` call regardless. This capture caught the throttle and the actual accepted sample landing on two DIFFERENT events half a second apart with wildly different scales (2.686m raw vs. the ratio-0.96 one that actually got applied) — meaning the applied transform's rotation was invisible in the log by construction, not by bad luck.

**Fix (`ImageTargetAnchorSource.ts`, diagnostic only):** `cameraDiagnosticLine()` — already run exactly once per applied pose, never throttled — now also prints the anchor's actual resulting quaternion AND its Euler-angle (yaw/pitch/roll, degrees) equivalent, purely for human eyeballing "does this look tilted" without doing mental quaternion math. This guarantees the NEXT capture shows precisely what rotation is in effect for every accepted sample, closing the gap that made this session unable to answer its own question.

**Verified in software:** `npm run typecheck`/`build`/`test` clean, 53/53 (diagnostic-only addition, no tracking decision changed, nothing new to assert). **Next physical step:** repeat the walkaround; this time the log will show the accepted rotation directly. Compare across multiple accepted samples of the SAME plaque (`front`) if possible — identical Euler angles across samples points at `TARGET_FRAME_TO_WORLD_FIX`/mount-angle (hypothesis (a), §32); noticeably different angles per sample points at rotation noise the gate doesn't currently filter (hypothesis (b)).

---

## 35. The rotation question, finally answered: it's yaw drift, the same root phenomenon as position/scale — NOT a wrong constant. Plus the actual critical fix this pass: proving and correcting the first-scan stillness problem

**Rotation question resolved with `docs/log-7.txt`'s new Euler-angle diagnostic (§34).** Three accepted samples of the SAME `front` plaque across one continuous session:

| moment | yaw | pitch | roll |
|---|---:|---:|---:|
| bootstrap (+7.2s) | 20.4° | 2.1° | 7.0° |
| first trustworthy (+60.3s) | 72.7° | -0.7° | 2.7° |
| re-detection (+117.0s) | 4.3° | 2.0° | 6.7° |

**Pitch and roll — exactly the two axes `TARGET_FRAME_TO_WORLD_FIX` and the flat-mount assumption govern — are small and mutually consistent (within a few degrees, centered near 0°) across all three samples.** This is direct, on-device evidence that the fixed glue rotation and the flat-mount decision (§18) are basically correct; hypothesis (a) from §32 (a wrong constant or mount angle) is not supported. **Yaw, by contrast, swings across a 68° range with no consistency at all between samples of the literal same, physically unmoving plaque.** Yaw (heading around the vertical axis) is also the textbook weak axis for monocular VIO — there is no absolute compass reference, so accumulated heading drift is one of the best-documented failure modes of this class of tracking, not a surprising or unusual result. **Conclusion: the reported "diagonal tilt" is the SAME root SLAM world-frame drift this file has already characterized in position (§27/§30) and scale (§32/§33), now confirmed to also affect orientation.** It is not a separate bug, and not something a rotation-plausibility gate on top of the existing scale gate would fix in kind — it needs the same category of mitigation already identified for the other axes (periodic re-grounding via re-detection, which already applies to rotation exactly as it does to position, since `applyPose()` composes and freezes all three together).

**The actual critical fix this pass — proven, not guessed, and directly requested:** the person testing named "over 30 seconds to complete the very first scan" as unacceptable now that lighting/reflection are no longer limiting factors (§32/§33's matte-surface fix). `log-7.txt` proves exactly why, with the periodic camera-position log (§29) as the smoking gun: from `+17.8s` (the moment `trackingStatus` reached `NORMAL`) to `+61.2s` — **43 straight seconds** — the camera's logged position barely moved at all (sub-3cm total drift across the whole window), while the scale-mismatch ratio smoothly decayed from ~1.6 down to ~0.1 and only climbed back to an accepted 0.89 the moment real motion resumed at `+62.3s`. The existing "Still locking on — try moving your phone slightly closer, then farther" coaching hint (§20) was ALREADY on screen for the entire 43-second stillness window (it fires once, 2.5s after acquire()) — the text was visible the whole time and evidently not compelling enough to change behavior. The likely reason, not just a guess: once someone believes they've "found" the plaque, the instinctive reaction is to hold the phone extra-still to keep it "locked" — the exact opposite of what absolute-scale convergence needs.

**Fix (`main.ts`), UX only, no tracking/gate logic touched:** a stillness monitor runs every second alongside the existing coaching timer, comparing the live camera position (`EightWallSession.getCameraPosition()`, already exposed for diagnostics in §27) against its value ~1 second earlier. If the camera has moved less than 4cm for 6 continuous seconds, the hint escalates to a more direct message naming the actual problem: *"Keep your phone moving — holding it still is why this is taking a while. Try a slow, repeated closer-then-farther motion, or move it side to side."* This reacts to PROVEN inaction instead of firing once on a blind timer regardless of what the user is actually doing. `whenStable()` remains the only thing that ever reveals the scene — this only changes what coaching text is shown and when.

**Verified in software:** `npm run typecheck`/`build`/`test` clean, 53/53 (UI-only change; no existing test exercises `main.ts`'s coaching timers, nothing new to assert against). **Not verifiable in software, the actual test of this fix:** whether escalating to a more direct, stillness-aware message measurably shortens real first-scan times — the only way to know is the next physical capture, watching whether the camera-position log shows motion resuming sooner after the escalated text appears than it did here.

---

## 36. `docs/log-8.txt`: the stillness message never appeared, and a much worse root cause than stillness — 115 seconds stuck in LIMITED, and the worst yaw error yet on the final applied pose

Reported: this capture's first lock took noticeably longer than usual, the §35 "keep your phone moving" escalation never appeared, and once the model finally rendered, its orientation was wrong.

**Why the message never appeared: the position telemetry the stillness check relies on is only meaningful once `trackingStatus` is `NORMAL`, and this capture never reached `NORMAL` until +115.1s.** Every prior capture (log-1 through log-7) reached `NORMAL` within ~17–18s of `onStart`. This one sat in `LIMITED reason=INITIALIZING` for 115 STRAIGHT seconds. During that entire window, the periodic camera-position log (§27/§29) shows the position swinging by whole meters between consecutive 1-second samples — e.g. `(-0.83, 2.21, 0.32)` to `(-1.29, 3.17, 0.50)` a few seconds later, later `(0.14, 10.95, 3.26)` to `(-1.04, 13.93, 5.63)` one second apart. Real hand motion does not look like this; this is SLAM/VIO position-estimate noise from a tracker that has not converged yet. Because the §35 stillness check only measures whether position moved more than 4cm between samples, this noise reads as constant "movement" — so the stillness monitor correctly stayed silent (it was never actually true that the phone was being held still), but nothing else was watching for THIS failure mode, so the person testing was left looking at a static "Still locking on…" hint for nearly two minutes with no escalation at all.

**The moment `trackingStatus` did flip to `NORMAL` (+115.1s), the logged camera position reset to `(0.00, 1.00, 0.00)`** — the same coordinates 8th Wall uses at a fresh session start (compare to `+10.1s` at the top of this same log, before anything had moved: `(0.00, 1.00, 0.00)`). This is consistent with the engine re-establishing its world-tracking origin once it finally achieves a stable lock, rather than the badly-drifted `LIMITED`-phase estimate just being relabeled `NORMAL`.

**The eventual accepted anchor pose is the worst orientation error recorded yet.** The first trustworthy sample wasn't accepted until +151.1s (ratio=1.12), by which point another 36 seconds of tracked motion had passed since reaching `NORMAL`. Its diagnostic line: `anchor-rot=(-0.03, 0.92, -0.00, -0.38) euler(YXZ, deg)=(yaw=-135.0, pitch=1.6, roll=-2.7)`. Pitch and roll are consistent with every prior reading (small, near 0°) — the mount-angle constant is fine, again. Yaw at **-135.0°** is by far the largest error this file has recorded (§35's log-7 range was 4–73°). This is not a new failure mode — it is direct, mechanical confirmation of what §35 already predicted: **the longer SLAM takes to reach a stable lock, the more yaw drift accumulates before the anchor ever freezes**, because nothing corrects orientation until the first trustworthy sample lands. A 151-second cold start with 115 of those seconds spent in unconverged `LIMITED` tracking is exactly the condition that produces a wildly wrong first orientation — the code did what it was designed to do (freeze onto the first trustworthy sample), but that sample inherited two and a half minutes of unconstrained drift.

**Fix, `main.ts` (§36):** added a second, independent escalation to the same interval that already drives the §35 stillness check — this one fires on wall-clock elapsed time alone (`NOT_NORMAL_ESCALATION_AFTER_MS = 20000`, chosen well above the ~17-18s baseline seen in every prior normal capture), triggered only while `trackingStatus !== 'NORMAL'`, independent of the (meaningless, pre-`NORMAL`) position telemetry. It shows a message naming the actual failure mode — world tracking itself not initializing — rather than the closer/farther text, which was already on screen this whole time and was not the problem. The §35 stillness check itself is now explicitly gated on `trackingStatus === 'NORMAL'` (it returns early otherwise) — before this capture that gate was implicit and accidental (the noisy pre-`NORMAL` positions just happened to always register as "movement"); making it explicit documents why position isn't a valid signal yet in that phase.

**What this does NOT fix:** why tracking took 115 seconds to initialize in the first place. That is an environment/procedure question this log alone can't answer conclusively — worth comparing against what changed physically for this capture (distance held from the plaque during the initial scan, how much of the frame was filled by the plain matte cloth vs. other textured objects, lighting) on the next attempt. A plausible contributing hypothesis: the matte tablecloth introduced in §32 to kill glare is, by design, a large, low-texture, single-color surface — exactly what a monocular VIO tracker's own feature tracking (separate from image-target recognition) struggles with if it dominates the frame during the initial scan. Worth testing: keep some textured object (the table's edge, a hand, a patterned object) in frame during the first few seconds after tapping "Start AR," before settling on the plaque.

**Verified in software:** `npm run typecheck`/`build`/`test` clean, 53/53 (UI-only change, no existing test exercises `main.ts`'s coaching timers). **Next physical step:** repeat the scan; confirm the new "taking longer than usual" message appears if `NORMAL` isn't reached within ~20s, and try keeping more textured surfaces in frame during the initial scan to test the low-texture hypothesis above.

---

## 37. `docs/log-9.txt`: the real remaining defect isn't scale, it's ROTATION — a single sample can be scale-perfect and still wildly wrong, and the trust gate never checked that. Fix: require two agreeing samples before applying any pose.

Two back-to-back physical captures in the same log file, WITH textured objects placed near the plaques (testing §36's low-texture hypothesis) — and both exposed the same defect from a different angle.

**Capture 1: the reveal itself flip-flopped 70+ degrees in a quarter of a second.** The first-ever trustworthy sample (the one that reveals the anchor) landed with `anchor-rot=... euler(YXZ, deg)=(yaw=134.2, ...)`. 0.23 SECONDS later, a second `FOUND "site-tracking-front"` event arrived — a discrete re-detection, both scale-plausible (ratio 0.96) and `trackingStatus=NORMAL`, exactly like the first — and it was accepted too, per §26's design (re-detections always re-ground). Its yaw: **-155.8°**. Two consecutive detections of the literal same, physically unmoving plaque, both individually passing every existing check, disagreed by roughly 70° (the short way around a 360° wrap). Reported symptom: "el modelo renderizó completamente fuera del ancla y de orientación."

**Capture 2: perfect scale, still catastrophically wrong rotation.** The second test's sole reveal-triggering sample had `ratio=1.03` — about as good as a scale reading gets — six full seconds into stable `NORMAL` tracking (not a cold-start artifact). Its yaw: **-114.3°**. Reported symptom: "renderizó más próximo al ancla y la rotación, sin embargo visiblemente fuera de las dimensiones delimitadas" — a rotated-but-correctly-scaled model extends past where its real footprint should be, which is exactly what a large rectangular model rotated 100°+ off its true heading looks like.

**Root cause: `isSampleTrustworthy()` / the `found`-case trust gate has ONLY ever checked scale plausibility and `trackingStatus`. Neither says anything about whether the ROTATION component of that same sample is any good.** Both captures prove 8th Wall's single-frame image-target orientation estimate can be noisy independent of how good the translation/scale reading is — a scale-perfect, `NORMAL`-tracked sample is not proof its rotation converged. Unlike scale (checked against `physicalTargetWidthMeters`, an external ground truth), there is no equivalent ground truth for yaw to validate a single sample against.

**Fix (`ImageTargetAnchorSource.ts`, §37): temporal agreement instead of a single-sample check.** A sample that passes the existing scale+trackingStatus gate is no longer applied immediately. It's held as a `pendingCandidate` (target name, yaw, timestamp); only once a SECOND trustworthy sample for the SAME target arrives within `YAW_CONFIRM_WINDOW_MS` (1000ms) and its yaw agrees within `YAW_CONFIRM_TOLERANCE_DEG` (20°) does the CONFIRMING sample actually get applied. This directly filters both captures' failure shape without ever having to decide which of two divergent readings was "correct" — it just refuses to act on an isolated one. Applies to every call site that used to apply unconditionally on the first pass: the pre-freeze `'updated'` stream, and every `'found'` re-detection (both new-target and repeat) — the unconditional bootstrap sample (the very first sighting of any plaque, while still hidden) is deliberately untouched, since it must never hang waiting for confirmation.

**Cost:** while a target stays in view, `'updated'` fires every real tracked frame (only its console.log is throttled to 1/s), so in the common case confirmation lands on the very next frame — on the order of tens of milliseconds, not seconds. It only meaningfully delays anything in the exact pathological case this exists to catch: an isolated bad reading with no immediate corroborating follow-up.
...
## 39. `docs/log-11.txt`: §37/§38 confirmed working — tilt now excellent, yaw still drifts as expected — and the newly-dominant symptom is the long-flagged global-rescale limitation (§27 hypothesis (b)), not a gate bug

Reported: lock took a while again, the tilt looks more accurate now but rotation (heading) still doesn't, and scale was lost rapidly until the model looked very shrunk, "como cayendo en el vacío."

**§38's pitch/roll plausibility gate is directly confirmed working on real hardware.** Multiple pre-reveal samples were rejected outright with the exact new warning line: `rotation implausible: pitch=-16.4°, roll=-6.6°`, `pitch=-16.1°, roll=-9.1°`, `pitch=-20.3°, roll=-9.4°` — precisely the kind of tilted, corrupt reading that used to sail through and, per §38, accumulate into a visible lean. **The pose that ultimately got applied and frozen has pitch=-1.0°, roll=3.8°** — both excellent, an order of magnitude better than log-10's final frozen pitch of 35.5°. This is a direct, mechanical confirmation that §38 fixed the reported tilt, matching what was seen: "la inclinación parece más exacta."

**Yaw (-28.9° on the frozen pose) is still not corrected — expected, not a regression.** As established since §35, yaw has no ground truth to check a sample against (monocular VIO's textbook weak axis, no compass reference), unlike pitch/roll which are checked against the known-flat mount. "no así la rotación" is the same already-documented limitation, working as understood — §37's consensus gate only protects against a single wildly-wrong outlier landing directly on the frozen anchor; it cannot make the underlying heading estimate itself accurate. (A genuine fix for yaw would require an independent absolute-heading source — e.g. the device compass/magnetometer via `DeviceOrientationEvent` — which is a materially different, bigger avenue than anything implemented so far and not attempted here.)

**The new dominant symptom — scale collapsing progressively after reveal — is the anchor's transform NEVER changing, while every subsequent re-detection attempt is correctly REJECTED.** After the reveal at +41.6s, four more `FOUND "site-tracking-front"` events arrived at +42.0s, +60.7s, +70.9s, and +74.8s — every single one rejected with `re-detection REJECTED (scale implausible)`, at ratios 0.60 (0.018m read vs 0.03m declared), 3.18, and 4.17. **No "trustworthy pose accepted"/`onOriginChanged` line appears anywhere after the reveal — the anchor's own position/rotation/scale genuinely never moved again for the rest of the session.** Since `anchorScaleForEvent()` always returns 1 (§F: `scale:'absolute'` means the anchor never scales itself), a visibly shrinking model with an untouched anchor can only mean the ENGINE's own internal definition of "one meter" is drifting underneath it — exactly the standing hypothesis (b) from §27 ("the CAMERA's own SLAM pose/absolute-scale estimate silently rescaling underneath an otherwise-correct, untouched anchor... not something any per-sample ratio check on the TRACKED IMAGE can catch, since a global rescale moves the image's apparent size and the camera's own position by the same factor"), now caught directly: the periodic camera-position log shows camera height climbing from ~0.5m to ~5m across roughly 15 seconds late in the session (+42.7s to +58.9s) — the same runaway-height signature already characterized as SLAM position/scale-calibration drift in §27/§30/§32, just no longer masked by the (now largely fixed) rotation bugs.

**This is not a gate bug.** The trust gate is doing exactly what it's designed to do: a re-detection whose scale doesn't match the ORIGINAL calibration is rejected, protecting the anchor from a bad individual reading. The problem is that once the engine's own internal scale has drifted, EVERY subsequent reading reflects the new (drifted) reality and will keep failing the same check relative to the original `physicalTargetWidthMeters` — there is no way to tell "this one sample is corrupt" apart from "the whole world genuinely rescaled" using a single-sample ratio check, and the three rejected ratios here (0.60, 3.18, 4.17) don't even agree with EACH OTHER, meaning the engine's scale estimate is still actively wandering, not settled on some new-but-consistent value a recalibration step could adopt. Periodic re-detection (§26) remains the correct mechanism to recover — it will re-ground the anchor correctly the moment the SLAM's own scale estimate happens to land back near the original calibration — but that depends on the engine's own estimate re-settling, which nothing in application code can force.

**Open product question, not yet acted on:** since the fix (a fresh re-detection) already exists and works whenever the underlying estimate cooperates, the most promising REMAINING lever is inviting the user to re-scan a plaque partway through a session, not only at cold start — today's coaching UX (`main.ts`) only ever runs before the first reveal; nothing nudges a "look back at a plaque" during an already-revealed session where the content visibly starts to look wrong. This is a UX/product decision (what the affordance looks like, when it should trigger, whether it should be persistent or condition-triggered) rather than a bug fix, and hasn't been implemented — raised with the person testing before building anything.

**Verified in software:** no code changed this pass — this is a diagnosis-only entry confirming §37/§38 on real hardware and re-confirming §27's already-documented hypothesis (b) as the current dominant symptom.
