...
def derive_crop_rectangle(msp):
    ...
    v_sign). Origin corner, u_sign, and v_sign: see
    extract_site_terrain.py's identical function — both scripts must
    derive the exact same origin/axes for terrain and buildings to align,
    kept as duplicated logic rather than a shared import (matches this
    project's existing split-script convention)."""
...
def main():
    ...
    for e in raw:
        ...
        uv = [to_local_uv(x, y, ox, oy, angle, u_sign, v_sign) for x, y in pts_dwg]
