...
// front'/'-back'/'-left'/'-right' are 4 single-image-target 8th Wall entries
// (same shape as 'site-front'/etc. above, i.e. re-center the WHOLE scene on
// themselves — no shared-corner offset composition) for testing the abstract
// Voronoi tracking-target candidates (tools/build_site_tracking_targets.py)
// one side at a time. Promoted into the production 'site' entry's targets[]
// on 2026-08-25 (AR_SYSTEM.md §G) — these 4 standalone entries remain a
// validation harness only, not the production path. On-device test of
// 'site-tracking-front' surfaced visible spin/scale jitter, root-caused to
// ImageTargetAnchorSource.applyPose() applying every trustworthy sample's
// raw rotation/position with zero temporal filtering — FIXED 2026-08-26
// (One Euro filtering, tuned like MindAR's TRACKING_PROFILE_RIGID_ANCHOR;
// see ImageTargetAnchorSource.ts's own doc comment and AR_SYSTEM.md §G's
// 2026-08-26 entry for the full write-up, including a real multi-target
// switching bug found and fixed in the same pass). Flip ACTIVE_TARGET_ID to
// any of the 4 site-tracking-* ids to test one candidate in isolation.
